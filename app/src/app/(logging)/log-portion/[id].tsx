...
import { api, ApiError, MEALS, type Batch, type Meal } from '@/lib/api';
...
// Log a portion from the inventory (Add Food → Prepped → a batch): pick how
// many portions, watch the macro ring scale live, confirm the meal + day, and
// log it — decrementing the batch's stock in the same call. Mirrors the food
// detail/quantity screen, but the unit is whole portions, not grams.
export default function LogPortionScreen() {
  ...
  const maxPortions = batch?.portionsRemaining ?? 1;
  const scaled = batch
    ? {
        kcal: batch.perPortionMacros.kcal * portions,
        protein: batch.perPortionMacros.protein * portions,
        carbs: batch.perPortionMacros.carbs * portions,
        fat: batch.perPortionMacros.fat * portions,
      }
    : null;

  const step = (delta: number) => {
    setPortions((current) => Math.min(maxPortions, Math.max(1, current + delta)));
  };
  ...
  const onAdd = async () => {
    ...
    setSaving(true);
    try {
      await api.eatPortion(token, batch.id, { date, meal, portions });
    ...
    <ThemedView style={styles.container}>
      <SafeAreaView style={styles.safeArea} edges={['top', 'bottom']}>
        ...
        {loadError ? (
          ...
          <View style={styles.centered}>
            <ThemedText type="small" themeColor="textSecondary" style={styles.centeredText}>
              No portions left in this batch.
            </ThemedText>
          </View>
        ) : (
          <>
            <ScrollView
              ...
              {/* Quantity stepper (whole portions, bounded by stock). */}
              <View style={[styles.stepper, { backgroundColor: theme.surface, borderColor: theme.surfaceBorder }]}>
                <Pressable
                  ...
                  style={({ pressed }) => [
                    ...
                    (pressed || portions <= 1) && styles.pressed,
                  ]}>
                  <ThemedText style={[styles.stepLabel, { color: theme.textSecondary }]}>–</ThemedText>
                </Pressable>
                <View style={styles.stepperValue}>
                  <ThemedText style={[styles.portionsInput, { color: theme.text }]}>{portions}</ThemedText>
                  <ThemedText style={[styles.portionsUnit, { color: theme.textMuted }]}>
